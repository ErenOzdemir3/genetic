...
void Graph::generatesGraph()
{
	...
	for(int i = 0; i < size_edges; i++)
	{
		...
		if(src != dest)
		{
			City(vec[src], vec[dest]);
			City(vec[dest], vec[src]);
		}
	}
}
...
void Graph::City(int src, int dest) // add a edge
{
	map_edges[make_pair(src, dest)] ; // adds edge in the map
}
...
{	
	...
	for(it = map_edges.begin(); it != map_edges.end(); ++it)
		cout << it->first.first << " linked to vertex " << it->first.second << " with  " << it->second << endl;
}
